  ...
  tasks.forEach((task) => {
    ...
    task.result.groups.forEach((group) => {
      ...
      group.members.forEach((member) => {
        groupData.push([
          ...
          member.gender === 1 ? '男' : '女',
          member.isLeader ? '是' : '否',
          member.rankingPercent ?? '',
          member.major,
          member.initiativeScore,
          member.extroversionScore,
          member.learningStyleIntensity,
    ...
    task.result.groups.forEach((group) => {
      ...
      statsData.push([
        ...
        parseFloat(group.statistics.averageInitiativeScore.toFixed(2)),
        parseFloat(group.statistics.averageExtroversionScore.toFixed(2)),
        group.statistics.averageRankingPercent === null
          ? ''
          : parseFloat(group.statistics.averageRankingPercent.toFixed(2)),
        parseFloat(group.statistics.averageLearningStyleIntensity.toFixed(2)),
  ...
  return {
    ...
    groups: task.result.groups.map((group) => ({
      ...
      members: group.members.map((m) => ({
        ...
        isLeader: m.isLeader,
        learningStyleIntensity: m.learningStyleIntensity,
        initiativeScore: m.initiativeScore,
        extroversionScore: m.extroversionScore,
      })),

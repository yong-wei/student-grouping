...
% 302743594
% groupNum = ones(1,17)*6; groupNum([1:4,8,10]) = [5,5,5,5,5,7];
% 302728255
groupNum = ones(1,17)*6; groupNum([2,7,11:13]) = [5,5,5,5,7];
dataset = '302728255partialLS';
LSname = readtable([dataset,'.xlsx'],VariableNamingRule="preserve");
names = table2cell(LSname(:,3));
data = table2array(LSname(:,4:21));
numGroups = length(groupNum);
% save([dataset,'.mat'],'LSname','data','names');
...
% 根据需要提取相应的数据，302728255班只需要考虑没有分组的24人分为4组
...
newsol = ABC('StudentGroupingGRL',numStudents,200000,para);
load([dataset,'.mat']);
...
para.data = data;%(isnan(data(:,end)),:);
para.numGroups = numGroups;
% [f,g,studentAssign,groupInd] = StudentGroupingGRL(sol.OptimumLocation,para);
% 针对302728255班的临时性修复
% fixedPerson = sum(~isnan(data(:,end))); % 固定分组的人数
% studentAssign = [data(1:fixedPerson,end);studentAssign];
% maxGroup = max(data(:,end));   % 现有的最大分组数
% fixedGroup = cell(maxGroup,1);
% for jj = 1:maxGroup
%     fixedGroup{jj} = find(data(:,end)==jj);
% end
% for jj = 1:length(groupInd)
%     groupInd{jj} = groupInd{jj} + fixedPerson;
% end
% groupInd = [fixedGroup;groupInd];
% ----------------------------
% [~,ind] = sort(sol.OptimumLocation); % Get the grouping index
[dataFinal,labelFinal] = groupVis(data,groupInd,names);
...
% gstat = [g.sg,g.stg,g.gender,g.leader,g.major,g.rank,g.score];
% sheet2 = [{'组号','积极/沉思','感官/直觉','视觉/言语','顺序/全局','组内各维度标准差之和','组内女生数','组内组长数','组内专业数','组内排名和','组内总分'};...
...
% ind = groupInd';
% ind(ind==0) = [];
% ind = [ind(:);(length(ind(:))+1:size(data,1))'];
% ind = ind(:);
grid = [6,8];
pages = ceil(size(dataFinal,1)/(grid(1)*grid(2)));
showRank = 0;
% studentVisFace(data(ind,:),names(ind),grid,pages,showRank);
studentVisFace(dataFinal,labelFinal,grid,pages,showRank);
%% Test face
close all

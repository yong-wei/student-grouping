function [f,groupStat,studentAssign,groupInd] = StudentGroupingGRL(x,ini)
% G: Gender考虑性别
% R: Ranking考虑排名
% L: Leader考虑组长
if nargin > 1
...
for ii = 1:nPop
    ...
    for jj = 1:m
        ...
        score(jj) = sum(g(:,5)); % 组内总分
        sg(jj,:) = mean(g(:,6:9)); % 小组平均属性（各组越接近越好）
        stg(jj) = sum(std(g(:,6:9))); % 组内各维度标准差之和（越大越好）
...
%         stg(jj) = std(abs(sum(g))); % Standard dev of each group       
    ...
    f(ii) = std(gender) + std(leader) + std(rank)/2 + std(major)/2 + ...
        std(score) + 4*mean(std(sg)) + 4/sum(stg); % Standard dev of all groups
end
groupStat.gender = gender;
groupStat.leader = leader;
groupStat.rank   = rank;
groupStat.major  = major;
groupStat.score  = score;
groupStat.sg     = sg;
groupStat.stg    = stg;